...
async def SauceNAOSearch(url: str, mode: str, hide_img: bool) -> List[str]:
    ...
    if res and res.raw:
        ...
        if selected_res.similarity < 60:
            final_res.append(f"相似度 {selected_res.similarity}% 过低，自动使用其他方式搜索")
            final_res.extend(await ASCII2DSearch(url, hide_img))
            final_res.extend(await SoutubotSearch(url, True))  # Doujin is always NSFW so I decide to hide them all
            final_res.extend(await TraceMoeSearch(url, hide_img))
    else:
